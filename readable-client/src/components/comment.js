...
downvoteComment() {
  const comment_id = this.props.comment.id
  APIHelper.downvoteComment(comment.id).then(comment) => {
...
deleteComment () {
  if(window.confirm('You really want to delete?')) {
    const comment.id = this.props.comment.id
    APIHelper.deleteComment(comment.id).then(() => {
...
generateModal (comment) {
 ...
 function mapDispatchToProps (dispatch) {
   ...
   deleteComment: (coment_id) => dispatch(actions.deleteComment(comment_id)),
  }
 }
